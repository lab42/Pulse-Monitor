...
func sendStartupConfig() {
	...
	theme := validateTheme(viper.GetString("theme"))
	accent := validateAccent(viper.GetString("accent"))

	log.Printf("Applying startup config: theme=%s, accent=%s\n", theme, accent)
	...
	jsonData, err := json.Marshal(msg)
	if err != nil {
		log.Println("Error marshaling startup theme:", err)
		return
	}

	if _, err = port.Write(append(jsonData, '\n')); err != nil {
		log.Println("Error sending startup theme:", err)
	}
}
...
func validateTheme(theme string) string {
	...
	log.Printf("Invalid theme '%s', using 'light'\n", theme)
	return "light"
}

func validateAccent(accent string) string {
	accent = strings.ToLower(accent)
	validAccents := []string{
		"sapphire", "sky", "teal", "green", "peach",
		"maroon", "pink", "flamingo", "mauve", "lavender",
	}
	for _, valid := range validAccents {
		if accent == valid {
			return accent
		}
	}
	log.Printf("Invalid accent '%s', using 'sapphire'\n", accent)
	return "sapphire"
}
