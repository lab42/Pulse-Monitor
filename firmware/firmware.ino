...
// ---- Custom fonts (C -> C++ safe) ----
extern "C" {
    extern const lv_font_t comfortaa_40;
    extern const lv_font_t bootstrap_icons_80;
}

#define LV_FONT_DEFAULT &comfortaa_40
...
// LVGL 9.x - Changed display driver types
...
// LVGL 9.x - Updated flush callback signature
void flush_display(lv_display_t *disp_drv, const lv_area_t *area,
                   uint8_t *px_map) {
...
// LVGL 9.x - Updated indev callback signature
...
void create_ui() {
  ...
  lv_style_set_anim_duration(&cpu_bar_style_main, ANIM_TIME);
  ...
  lv_style_set_anim_duration(&cpu_bar_style_indic, ANIM_TIME);
  ...
  lv_style_set_anim_duration(&memory_bar_style_main, ANIM_TIME);
  ...
  lv_style_set_anim_duration(&memory_bar_style_indic, ANIM_TIME);
  ...
  lv_style_set_anim_duration(&disk_bar_style_main, ANIM_TIME);
  ...
  lv_style_set_anim_duration(&disk_bar_style_indic, ANIM_TIME);
  ...
  lv_style_set_anim_duration(&gpu_bar_style_main, ANIM_TIME);
  ...
  lv_style_set_anim_duration(&gpu_bar_style_indic, ANIM_TIME);

  // Main container
  main_container = lv_obj_create(scr);
  lv_obj_set_size(main_container, LCD_H_RES - (OUTER_MARGIN * 2),
                  LCD_V_RES - (OUTER_MARGIN * 2));
  ...
  const lv_coord_t row_height =
      (LCD_V_RES - (OUTER_MARGIN * 2) - (ROW_PADDING * 2) - (ROW_SPACING * 3)) /
      4;
  ...
  lv_obj_set_flex_align(cpu_row, LV_FLEX_ALIGN_START, LV_FLEX_ALIGN_CENTER,
                        LV_FLEX_ALIGN_CENTER);
  lv_obj_remove_flag(cpu_row, LV_OBJ_FLAG_SCROLLABLE);
  ...
  lv_obj_set_flex_align(memory_row, LV_FLEX_ALIGN_START, LV_FLEX_ALIGN_CENTER,
                        LV_FLEX_ALIGN_CENTER);
  lv_obj_remove_flag(memory_row, LV_OBJ_FLAG_SCROLLABLE);
  ...
  lv_obj_set_flex_align(disk_row, LV_FLEX_ALIGN_START, LV_FLEX_ALIGN_CENTER,
                        LV_FLEX_ALIGN_CENTER);
  lv_obj_remove_flag(disk_row, LV_OBJ_FLAG_SCROLLABLE);
  ...
  lv_obj_set_flex_align(gpu_row, LV_FLEX_ALIGN_START, LV_FLEX_ALIGN_CENTER,
                        LV_FLEX_ALIGN_CENTER);
  lv_obj_remove_flag(gpu_row, LV_OBJ_FLAG_SCROLLABLE);
  ...
  lv_obj_set_flex_align(network_row, LV_FLEX_ALIGN_SPACE_EVENLY,
                        LV_FLEX_ALIGN_CENTER, LV_FLEX_ALIGN_CENTER);
  lv_obj_remove_flag(network_row, LV_OBJ_FLAG_SCROLLABLE);
  ...
  lv_obj_set_flex_align(download_container, LV_FLEX_ALIGN_START,
                        LV_FLEX_ALIGN_CENTER, LV_FLEX_ALIGN_CENTER);
  ...
  lv_obj_set_flex_align(upload_container, LV_FLEX_ALIGN_START,
                        LV_FLEX_ALIGN_CENTER, LV_FLEX_ALIGN_CENTER);
...
>>>>>>> Stashed changes
  ...
  // LVGL 9.x - Simplified display creation with direct buffer allocation
  lv_color_t *buf1 = (lv_color_t *)heap_caps_malloc(
      LCD_H_RES * BUFFER_LINES * sizeof(lv_color_t), MALLOC_CAP_DMA);
  ...
  if (!buf1 || !buf2) {
    ...
    // LVGL 9.x - New display creation API
  ...
  // LVGL 9.x - New input device creation API
...
<<<<<<< Updated upstream
void updateUI(float cpu, float mem, float gpu, float up, float down, float disk);
void resetUI();

void loop() {
    // --- Handshake (ASCII only) ---
    if (!serialConnected && Serial.available()) {
        String line = Serial.readStringUntil('\n');
        line.trim();

        if (line == "ID:ed1d2a7c8af14a27b77b1c127d806aed") {
...
<<<<<<< Updated upstream
    // --- MessagePack stream decoding ---
    if (serialConnected && Serial.available()) {
        DynamicJsonDocument doc(512);
        DeserializationError err = deserializeMsgPack(doc, Serial);
...
=======
        ...
        if (!error) {
          ...
          float mem =
              doc.containsKey("memory") ? doc["memory"].as<float>() : 0.0;
          float gpu = doc.containsKey("gpu") ? doc["gpu"].as<float>() : 0.0;
          float up =
              doc.containsKey("upload") ? doc["upload"].as<float>() : 0.0;
          float down =
              doc.containsKey("download") ? doc["download"].as<float>() : 0.0;
...
<<<<<<< Updated upstream
    // --- Connection timeout ---
    if (serialConnected && millis() - lastDataTime > CONNECTION_TIMEOUT) {
